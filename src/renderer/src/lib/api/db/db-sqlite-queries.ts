...
CREATE TABLE IF NOT EXISTS "${TABLE_NAME_FILTERS}" (
	"id"	INTEGER,
	"name"	TEXT NOT NULL,
	"notes"	TEXT,
	PRIMARY KEY("id" AUTOINCREMENT)
...
CREATE TABLE IF NOT EXISTS "${TABLE_NAME_NODES}" (
	...
	FOREIGN KEY("uiResponseText") REFERENCES "${TABLE_NAME_LOCALIZATIONS}",
	...
	FOREIGN KEY("link") REFERENCES "${TABLE_NAME_NODES}",
	PRIMARY KEY("id" AUTOINCREMENT)
);
`;
...
CREATE TABLE IF NOT EXISTS "${TABLE_NAME_EDGES}" (
	...
	FOREIGN KEY("parent") REFERENCES "${TABLE_NAME_CONVERSATIONS}",
	FOREIGN KEY("source") REFERENCES "${TABLE_NAME_NODES}",
	FOREIGN KEY("target") REFERENCES "${TABLE_NAME_NODES}",
...
BEGIN TRANSACTION;
INSERT OR IGNORE INTO ${TABLE_NAME_ROUTINES} (id, name, code, type, isSystemCreated) VALUES (${rowIndex++}, 'Import Statements', '', ${ROUTINE_TYPE_ID_IMPORTS}, true);
COMMIT;
`;
// Conversations
// Locales
